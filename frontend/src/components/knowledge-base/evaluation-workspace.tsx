...
export function EvaluationWorkspace({ kbId }: EvaluationWorkspaceProps) {
  ...
  const avgRelevance = evaluations.length
    ? evaluations.reduce((sum, item) => sum + item.relevanceScore, 0) / evaluations.length
    : 0;
  const avgFaithfulness = evaluations.length
    ? evaluations.reduce((sum, item) => sum + item.faithfulnessScore, 0) / evaluations.length
    : 0;
  const cannotAnswerCount = evaluations.filter((item) => item.cannotAnswer).length;
    ...
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-lg border border-[#7C3AED]/24 bg-[radial-gradient(circle_at_88%_12%,rgba(124,58,237,0.16),transparent_34%),linear-gradient(135deg,#F4F0FF,#FFFFFF_58%)] px-6 py-5 shadow-panel">
        <div className="relative flex items-start justify-between gap-4">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[#6D28D9]">Evaluation Lab</p>
          ...
          <button className="inline-flex h-9 items-center gap-1.5 rounded-md bg-gradient-to-r from-[#7C3AED] to-[#EC4899] px-3.5 text-[13px] font-medium text-ink-inverse shadow-[0_10px_24px_rgba(124,58,237,0.22)] hover:brightness-95">
            新建实验
          </button>
        </div>
      </div>
